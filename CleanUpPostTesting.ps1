...
$x = Get-AzResourceGroup
$y = $x | where-object { $_.ResourceGroupName -like "test*" }
foreach ($z in $y) { $z.ResourceGroupName; Remove-AzResourceGroup -Name $z.ResourceGroupName -Force -AsJob }


$x = Get-AzResourceGroup
$y = $x | where-object { $_.ResourceGroupName -like "*ScenarioValidation*" }
foreach ($z in $y) { $z.ResourceGroupName; Remove-AzResourceGroup -Name $z.ResourceGroupName -Force -AsJob }



$x = Get-AzResourceGroup
$y = $x | where-object { $_.ResourceGroupName -like "*ResourceGroupToMove*" }
foreach ($z in $y) { $z.ResourceGroupName; Remove-AzResourceGroup -Name $z.ResourceGroupName -Force -AsJob }
